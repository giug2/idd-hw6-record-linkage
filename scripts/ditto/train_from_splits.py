"""
DITTO Training from dataset/splits/ - VERSIONE CORRETTA
========================================================
Questo script:
1. Legge i dataset da dataset/splits/ (train.csv, validation.csv, test.csv)
2. GENERA COPPIE NEGATIVE per bilanciare il dataset (fondamentale!)
3. Applica le strategie di blocking B1 e B2
4. Crea dataset nel formato DITTO
5. Addestra i 6 modelli con PREDIZIONI REALI (non Jaccard!)
6. Salva i modelli in output/ditto/modelli/
7. Sovrascrive i dataset in output/ditto/ditto_dataset/
"""
...
def get_model_prefix(model, length=2) -> Optional[str]:
    """Estrae il prefisso del modello."""
...
def get_blocking_key_B1_craig(row: pd.Series) -> Optional[str]:
    """B1: brand + year for Craigslist record"""
...
def get_blocking_key_B1_us(row: pd.Series) -> Optional[str]:
    """B1: brand + year for US Cars record"""
...
def get_blocking_key_B2_craig(row: pd.Series) -> Optional[str]:
    """B2: brand + model_prefix for Craigslist record"""
...
def get_blocking_key_B2_us(row: pd.Series) -> Optional[str]:
    """B2: brand + model_prefix for US Cars record"""
...
def create_ditto_file(df: pd.DataFrame, pipeline: str, blocking: str, output_file: str) -> Tuple[int, int]:
    """
    Crea un file DITTO (tsv: text1 \t text2 \t label).
    Applica il blocking per filtrare le coppie POSITIVE.
    """
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        for idx, row in df.iterrows():
            ...
            # Calcola le blocking keys
            if blocking == 'B1':
                craig_key = get_blocking_key_B1_craig(row)
                us_key = get_blocking_key_B1_us(row)
            elif blocking == 'B2':
            ...
            if label == 1:
                ...
                if not (craig_key and us_key and craig_key == us_key):
                    continue
...
def prepare_all_datasets():
    """Prepara tutti i dataset per le 6 configurazioni."""
    print("="*70)
    print("PREPARAZIONE DATASET CON COPPIE NEGATIVE")
    print("="*70)
    ...
    # Crea dataset per ogni configurazione
    results = {}
    
    for pipeline in PIPELINES.keys():
        for blocking in ['B1', 'B2']:
            config_name = f"{pipeline}_{blocking}"
            print(f"\n--- {config_name} ---")
            ...
            # Crea files
            train_m, train_nm = create_ditto_file(
                train_balanced, pipeline, blocking,
                str(config_dir / "train.txt")
            )
            valid_m, valid_nm = create_ditto_file(
                valid_balanced, pipeline, blocking,
                str(config_dir / "valid.txt")
            )
            test_m, test_nm = create_ditto_file(
                test_balanced, pipeline, blocking,
...
def save_results(results: Dict, output_file: str):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write("="*95 + "\n")
        f.write("DITTO TRAINING RESULTS - REAL MODEL PREDICTIONS\n")
        f.write("Training con predizioni REALI del modello DITTO (non Jaccard)\n")
...
def main():
    ...
    print("\n" + "="*80)
    print("DITTO TRAINING FROM SPLITS - VERSIONE CORRETTA")
    print("="*80)
    ...
    print("\n" + "="*80)
    print("STEP 2: ADDESTRAMENTO MODELLI")
    print("="*80)
    ...
    # Step 3: Save results
    output_file = OUTPUT_DIR / "ditto_results.txt"
    ...
    print(f"Device: {'GPU (DirectML)' if GPU_AVAILABLE else 'CPU'}")
    print(f"Configurations: {len(results)}/6 completed")
    print("="*80)
