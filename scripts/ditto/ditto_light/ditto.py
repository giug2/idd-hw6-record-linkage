...
def train_step(train_iter, model, optimizer, scheduler, hp, scaler=None):
    ...
    for i, batch in enumerate(train_iter):
        ...
        if hp.fp16 and hp.device == 'cuda':
            ...
            # Standard backprop for CPU and other devices (e.g., DirectML)
            loss.backward()
            optimizer.step()
        scheduler.step()
        if i % 10 == 0:
...
def train(trainset, validset, testset, run_tag, hp):
    ...
    # Support any PyTorch device (cuda, cpu, directml, etc)
    try:
        model = model.to(hp.device)
        print(f"Model moved to device: {hp.device}")
    except Exception as e:
        print(f"Warning: could not move to {hp.device}, falling back to CPU: {e}")
        model = model.to('cpu')
        hp.device = 'cpu'
