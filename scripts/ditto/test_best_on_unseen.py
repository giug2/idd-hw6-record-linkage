"""
Test del miglior modello compatibile sul dataset unseen
========================================================
Dato che unseen_final.csv ha solo: brand, model, year, price, description
Possiamo usare solo P3_minimal_fast (brand, model, year)

Il modello con le migliori prestazioni compatibili è P3_minimal_fast_B2 (F1=0.9983)
"""
...
# Configurazione - P3_minimal_fast_B2 è il miglior modello compatibile con unseen
...
def prepare_unseen_dataset(unseen_path: str, output_path: str):
    """
    Prepara il dataset unseen nel formato DITTO.
    Tutte le coppie in unseen_final.csv sono match (stesso VIN) -> label=1
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        for idx, row in df.iterrows():
            # Blocking B2
            craig_key = get_blocking_key_B2(row, 'craig')
            us_key = get_blocking_key_B2(row, 'us')
...
def evaluate_on_unseen(model, threshold, test_file: str, device, batch_size: int = 32):
    ...
    # Nel dataset unseen tutte le coppie sono veri match (stesso VIN)
    # Quindi label=1 per tutte
...
def save_results(results: dict, output_file: str, model_name: str):
    """Salva i risultati nel file txt."""
    with open(output_file, 'w', encoding='utf-8') as f:
        device_str = "GPU (AMD DirectML)" if GPU_AVAILABLE else "CPU"
        
        f.write("="*80 + "\n")
        f.write("DITTO UNSEEN TEST RESULTS\n")
        ...
        f.write("-"*80 + "\n")
        f.write(f"  Source: dataset/unseen_final.csv\n")
        ...
        f.write(f"  Actual Non-Match     {cm[0][0] if cm.shape[0] > 0 else 0:<10}   {cm[0][1] if cm.shape[0] > 0 else 0}\n")
        f.write(f"  Actual Match         {cm[1][0] if cm.shape[0] > 1 else 0:<10}   {cm[1][1] if cm.shape[0] > 1 else 0}\n\n")
        ...
        f.write("NOTE:\n")
        f.write("-"*80 + "\n")
        f.write("  Il dataset unseen_final.csv contiene solo coppie con lo stesso VIN,\n")
...
def main():
    print("\n" + "="*80)
    print("TEST MIGLIOR MODELLO SU DATASET UNSEEN")
    print("="*80)
    ...
    unseen_ditto = OUTPUT_DITTO_DIR / "ditto_dataset" / f"{MODEL_NAME}_unseen" / "test.txt"
    model_dir = MODELS_DIR / MODEL_NAME
    output_file = OUTPUT_DITTO_DIR / "unseen_test_results.txt"
    ...
    print("\n" + "-"*80)
    print("STEP 1: Preparazione dataset unseen")
    print("-"*80)
    ...
    print("\n" + "-"*80)
    print("STEP 2: Caricamento modello")
    print("-"*80)
    ...
    # Step 3: Valuta
    print("\n" + "-"*80)
    print("STEP 3: Valutazione su unseen")
    print("-"*80)
    ...
    print("-"*80)
    save_results(results, str(output_file), MODEL_NAME)
    print(f"Risultati salvati in: {output_file}")
    
    # Stampa riepilogo
    print("\n" + "="*80)
    print("RISULTATI UNSEEN TEST")
    print("="*80)
    print(f"Modello:    {MODEL_NAME}")
